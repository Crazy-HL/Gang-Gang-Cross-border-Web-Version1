<template><div class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]"><section class="rounded-[2rem] border border-white/10 bg-panel/75 p-5"><label class="text-sm font-semibold text-white">搜索管理员任务<input v-model="keyword" class="mt-2 w-full rounded-2xl border border-white/10 bg-ink-2 px-4 py-3 text-slate-100" /></label><div class="mt-5 space-y-3"><button v-for="job in filteredJobs" :key="job.id" type="button" class="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left hover:border-gold/40" @click="selectedId = job.id"><div class="flex justify-between gap-4"><span class="font-bold text-white">{{ job.title }}</span><span :style="{ color: getRiskMeta(job.riskLevel).color }">{{ getRiskMeta(job.riskLevel).label }}</span></div><p class="mt-2 text-sm text-slate-400">用户：{{ job.ownerName }} · 任务 {{ job.id }}</p></button></div></section><aside class="rounded-[2rem] border border-gold/20 bg-gold/10 p-6"><h2 class="text-xl font-bold text-white">任务详情</h2><div v-if="selectedJob" class="mt-5 space-y-3 text-sm text-slate-300"><p>任务ID：{{ selectedJob.id }}</p><p>用户：{{ selectedJob.ownerName }}</p><p>品牌：{{ selectedJob.brand }}</p><p>市场：{{ selectedJob.market }}</p><div class="grid h-36 place-items-center rounded-2xl border border-gold/20 bg-ink-2 text-gold">原始文件缩略图</div></div></aside></div></template>
